...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func setupScoreLabel() {
        ...
        let bestItemScore = userDefaults.integer(forKey: "ITEM BEST")
    ...
    func setupItem(){
        ...
        let random_item_range = birdSize.height * 3
        
        // 下の壁のY軸下限位置(中央位置から下方向の最大振れ幅で下の壁を表示する位置)を計算
        let groundSize = SKTexture(imageNamed: "ground").size()
        let center = groundSize.height + (self.frame.size.height - groundSize.height) / 2
        let under_item_lowest_y = center - itemTexture.size().height / 2 - random_item_range / 2
        
        let createItemAnimation = SKAction.run({
            
            let item = SKNode()
            item.position = CGPoint(x: self.frame.size.width / 2 + itemTexture.size().width*3  , y: -self.frame.height / 8)
            item.zPosition = -60
            
            let random_item = CGFloat.random(in: 0 ..< random_item_range)
           
            let under_item_y = under_item_lowest_y + random_item
            ...
            underItem.physicsBody?.categoryBitMask = self.itemCategory    // ←追加
            ...
            itemScoreNode.position = CGPoint(x: self.frame.size.width + itemTexture.size().width / 2, y: self.frame.height / 2)
            itemScoreNode.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: underItem.size.width, height: self.frame.size.height))
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        let music = SKAction.playSoundFileNamed("cursor6.mp3", waitForCompletion: false)
        // ゲームオーバーのときは何もしない
        ...
        if (contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory || (contact.bodyB.categoryBitMask & scoreCategory) == scoreCategory {
            ...
            var bestItemScore = userDefaults.integer(forKey: "ITEM BEST")
            if itemScore > bestItemScore{
                bestItemScore = itemScore
                bestItemScoreLabelNode.text = "Best Item Score:\(bestItemScore)"
                userDefaults.set(bestItemScore, forKey: "ITEM BEST")
                userDefaults.synchronize()
            }
